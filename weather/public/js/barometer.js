( function( $ ) {
    ...
    Barometer.prototype = {
      ...
      createInputContainer: function () {
          const self = this;
          const input = $("<form id=\"barometer_form\"><label>"+ this.options.placeholder + "</label><input type=\"number\" min=\"0\" max=\"100\" value="+this.options.startvalue+" step=\""+this.options.steps+"\"/></form>");
      ...
      rotate: function ( degs ) {
            ...
            // Normalizing for soil prob values between 0-100 with -45 and 255 degrees in gaugge
            degs_norm = (280 * ((degs) / 100)) -45;
            console.log(degs_norm);
            console.log(degs);
            $arrow.css({ WebkitTransform: 'rotate(' + degs_norm  + 'deg)'});
            // For Mozilla browser: e.g. Firefox
            $arrow.css({ '-moz-transform': 'rotate(' + degs_norm + 'deg)'});
        },
